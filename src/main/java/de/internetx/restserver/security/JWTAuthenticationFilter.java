...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            AuthUser authReq = new ObjectMapper()
                    .readValue(request.getInputStream(), AuthUser.class);

            return authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(authReq.getLogin(),
                    authReq.getPassword(), new ArrayList<>()));
        } catch (IOException e) {
